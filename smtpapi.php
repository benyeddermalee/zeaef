...
if (!isset($_COOKIE['smtp'])) {
    echo json_encode([
        ...
        'error' => 'SMTP command cookie not provided.',
        'timestamp' => date('c')
    ]);
    exit;
}
...
$body_base = '';
$body_source = '`body` cookie';
if (!empty($body_ref)) {
    $body_source = $body_ref;
    if (preg_match('~^https?://~i', $body_ref) || is_file($body_ref)) {
        $body_base = @file_get_contents($body_ref);
    }
} elseif (isset($_COOKIE['body'])) {
    $body_base = $_COOKIE['body'];
}

if (empty($body_base)) {
    $final_report[] = ['timestamp' => date('c'), 'status' => 'REJECTED', 'error' => 'Email body is missing or could not be fetched.', 'details' => ['body_source_tried' => $body_source]];
    echo json_encode($final_report); exit;
}
...
foreach($recipients as $to) {
    ...
    $current_report['details'] = [
        'from_email_resolved' => $from_email,
        'from_name_resolved' => $from_name,
        'subject_resolved' => $subject,
        'bcc_list_resolved' => $bcc_recipients,
        'body_source' => $body_source,
        'content_type' => $is_html ? 'HTML' : 'Plain Text'
    ];
    ...
    if (strtolower($host) === 'localhost') {
        ...
        try {
            ...
            $current_report['details']['mailer_type'] = 'SMTP';
            $current_report['details']['smtp_host'] = $current_smtp['host'];
            $current_report['details']['smtp_port'] = $current_smtp['port'];
            $current_report['details']['smtp_user'] = $current_smtp['user'];
            $current_report['details']['smtp_encryption'] = $current_smtp['enc'] ?: 'None';
            $current_report['details']['from_as_login_flag'] = (bool)$use_from_as_login;
            $current_report['details']['final_sender_email'] = $login_email;

            $mailer->send();
            $current_report['status'] = 'SUCCESS';
            $current_report['details']['server_response'] = $mailer->ErrorInfo;
            $sent_count++;
        } catch (Exception $e) {
            $current_report['status'] = 'FAILED';
    ...
    if ((int)$pause_every > 0 && (int)$pause_for > 0 && $sent_count > 0 && $sent_count % (int)$pause_every === 0 && $sent_count < count($recipients)) {
        sleep((int)$pause_for);
    }
}
